...
export default function AdminProviderManager({ initialProviders }: { initialProviders: Provider[] }) {
  ...
  const handleDelete = async (providerId: string) => {
    if (!confirm(`Are you sure you want to delete the provider "${providerId}"? This will also delete all of its models.`)) return;
    setIsLoading(true);
  ...
  return (
    <div style={{ border: '1px solid #ddd', padding: '1rem', borderRadius: '8px', marginTop: '2rem' }}>
      ...
      <table style={{width: '100%', borderCollapse: 'collapse'}}>
        ...
        <tbody>
          {providers.map(p => (
            <tr key={p.id}><td>{p.id}</td><td>{p.name}</td><td>{p.base_url}</td><td>{p.auth_header}</td>
              <td><button onClick={() => handleDelete(p.id)} disabled={isLoading}>Delete</button></td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div style={{ background: '#f0fff0', padding: '1rem', marginTop: '1rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          <input placeholder="Auth Header (e.g., x-api-key)" value={newProvider.auth_header} onChange={e => setNewProvider({...newProvider, auth_header: e.target.value})} />
          <input placeholder="Auth Scheme (e.g., Bearer)" value={newProvider.auth_scheme || ''} onChange={e => setNewProvider({...newProvider, auth_scheme: e.target.value})} />
